...
public class LongestIncreasingSubsequence {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Longest Increasing Subsequence (Binary Search) : " + funcBinarySearch(a));
    }
    ...
    private static int funcBinarySearch(int[] a) {
        ...
        for (int i = 1; i < n; i++) {
            if (a[i] < temp[0]) {
                ...
                int j = findCeil(temp, -1, len - 1, a[i]);
                temp[j] = a[i];
            }
        }
        return len;
    ...
    private static int findCeil(int[] a, int l, int r, int key) {
        while (r - l > 1) {
            int m = (l + r) / 2;
            if (a[m] >= key) {
                r = m;
